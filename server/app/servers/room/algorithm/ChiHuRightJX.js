...
class ChiHuRight {
     ...
     * @returns {boolean}
     */
    use(seatId, action, pai, prevSeatId, can_mulhu){
        let self = this;

        can_mulhu = can_mulhu || false;
        /**
         * 检查操作的有效性
         * @type {string[]}
         */
        ...
        if (maxId === seatId)
        {
            ...
            //todo 报听情况不分座位优先级
            if ((action === alg.mj_wik_action.WIK_BAOTING) || ( can_mulhu && ((curAction.opt & alg.mj_wik_action.WIK_CHI_HU) === alg.mj_wik_action.WIK_CHI_HU )) )
            {
                return true;
            }
        }
        ...
        switch (action_mask)
        {
            ...
            case alg.right_type.gang:
                //if (right.opt_list.length > 0)
                {
                    ...
                    {
                        let item = right.actionPai[key];
                        if ((item.opt & (alg.mj_wik_action.WIK_ANGANG | alg.mj_wik_action.WIK_MINGGANG | alg.mj_wik_action.WIK_DIANGANG )) > 0)
                        {
                            return item.pai;
                        }
                    }
