...
class Algorithm_1 extends Poker {
    constructor() {
        super();
    }
     ...
     * @param {*} tipsCards
     */
    getZhuTypeAABBTipsCards(holds, tipsCards=true) {
        let zhus = this.getHoldsZhu(holds);
        if (zhus.length < this.firstCards.cards.length) {
            tipsCards.musts = tipsCards.musts.concat(zhus);
            let fus = this.getHoldsFu(holds);
            tipsCards.others = this.combination(fus, this.firstCards.cards.length-zhus.length);
        } else {
            let aabbs = this.getMaxLenAABB(zhus);
            /** 找到所有跟打出去的牌一样的拖拉机 */
            for (let i = 0; i < aabbs.length; i++) {
