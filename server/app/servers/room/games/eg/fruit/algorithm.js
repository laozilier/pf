...
class ChzAlgorithm {
     ...
     * @param xingInfo
     */
    checkWangZha(player_holds, player_inPokers, card, xingCard, xing, gameRules) {
        /** 王数量小于3个 */
        ...
        let maxObj = this.checkHu(holds, inPokers, null, xingCard, xing, gameRules, minHu, huType);
     ...
     * @param card
     */
    checkWangChuang(player_holds, player_inPokers, card, xingCard, xing, gameRules) {
        /** 王数量小于2个 */
        let wCount = this.calcCountInArray(player_holds, this.laizi);
        if (wCount < 2) {
            return;
     ...
     * @param card
     */
    checkWangDiao(player_holds, player_inPokers, card, xingCard, xing, gameRules) {
        /** 王数量小于1个 */
        let wCount = this.calcCountInArray(player_holds, this.laizi);
        if (wCount < 1) {
            return;
        ...
        let maxObj = this.checkHu(holds, inPokers, null, xingCard, xing, gameRules, 15, huType);
        if (!!maxObj) { maxObj.huCard = card; }
        return maxObj;
    }
     ...
     * @param xingInfo
     */
    checkHu (player_holds, player_inPokers, card, xingCard, xing, gameRules=[], minHu=15, huType=1) {
        let holds = [].concat(player_holds);
        let inPokers = JSON.parse(JSON.stringify(player_inPokers));
        let all = [];
        ...
        let ergunyi = gameRules[1];
        let redblack = gameRules[1];
...
let algorithm = new ChzAlgorithm();
module.exports = algorithm;

// console.log(algorithm.checkHu([algorithm.laizi,13, 14,14,14,14, 10, 10],[],
//     12, 15, 14));

// console.log(algorithm.checkWangDiao([15, 3, 8, 15, 10, 21, 9, 2, 15, 15, 21],
//     [{"v":4,"vs":[4,4,4],"t":2,"hu":1},{"v":14,"vs":[14,14,14],"t":2,"hu":3},{"v":1,"vs":[1,1,1,1],"t":3,"x":false,"p":false,"hu":6,"c":true}]
//     )
// );

// console.log(algorithm.checkQing([20, 17, 21, 9, 12, 7, 7, 7, 21, 9, 9],
//     [],
//     7));
// console.log(algorithm.checkXiao([1,1], 1));
// console.log(algorithm.checkJiao([1,1,1],
//     [
//         { v: 2, vs: [ 2, 2, 2, 2 ], t: 5, x: false, hu: 9, c: false },
//         { v: 3, vs: [ 3, 3, 3 ], t: 4, g: false, hu: 3 },
//         { v: 4, vs: [ 4, 4, 4 ], t: 2, g: false, hu: 1 },
//     ],
//     1));
// console.log(algorithm.checkPeng([1,1], [], 1));
// console.log(JSON.stringify(algorithm.checkChi([18,13,15,9,16,15,2,6,4,17,5,9,20,9,16,3,5,1,11,2], 15)));
// console.log(JSON.stringify(algorithm.checkChi([15,15,5,5,16,17], 15)));
// console.log(algorithm.checkPeng([10,13,10,16,13,11,20,10,7,16,3,10,17,1], 13));
