...
class Poker_sbf_algorithm extends require('../poker/Algorithm_1') {
    constructor () {
        super();

        this.zhus.push.apply(this.zhus, [9,22,35,48]);
        this.cardsType = require('./poker_sbf_enum').cardsType;
        this.scoreMap['52'] = 20;
        this.scoreMap['53'] = 30;
    };
                        ...
                        if (typeCards.length > 1) {
                            ...
                            if (typeCards.length == 0) {
                                ...
                                if (duizis.length > 0) {
                                    for (let i = 0; i < duizis.length; i++) {
                                        let duizi = duizis[i];
                                        let tempCardsData = this.getCardsData(duizi, true, true);
                                        if (this.compareCardsDatas(this.maxCards, tempCardsData)) {
                                            return;
                                        }
                                    }
...
let algorithm = new Poker_sbf_algorithm();
