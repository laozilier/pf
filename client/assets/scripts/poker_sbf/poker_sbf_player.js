...
cc.Class({
    ...
    onLoad() {
        ...
        this.liangCard = this.node.getChildByName('liangCard');   //玩家亮的牌或反的牌
        for (let i = 0; i < 2; i++) {
            let poker = cc.instantiate(this.pokerPrefab);
            this.liangCard.addChild(poker);
            poker.active = false;
        }
        this.initHolds();
    },
    ...
    addHolds(card) {
        ...
        if (Array.isArray(this._holds) && typeof card == 'number') {
            this._holds.push(card);
            this.checkHolds();
        }
    },
    ...
    hideLiangCard() {
        ...
        this.liangCard.children.forEach((poker)=>{
            poker.active = false;
        });
    },
    ...
    jiaozhu(tempZhu) {
        ...
        this.liangCard.children.forEach((poker, idx)=>{
            if (idx < tempZhu.length) {
                poker.active = true;
                this.pokerScript(poker).show(tempZhu[idx]);
            } else {
                poker.active = false;
            }
        });
