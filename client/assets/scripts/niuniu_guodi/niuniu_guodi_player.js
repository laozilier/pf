...
cc.Class({
    ...
    onLoad () {
        ...
        this._resorix = this.resultNode.children[0].x;
        this._resoriy = this.resultNode.children[0].y;
    ...
    set(data) {
        ...
        switch(cc.gameargs.status) {
            ...
            case cc.game_enum.status.WAIT_ROBS_OPEN:
                this.setRob(data.rob);
                break;
            case cc.game_enum.status.WAIT_BETS:
                this.showHolds();
                this.setBet(data.bet, data.shemen);
                break;
            case cc.game_enum.status.CUOPAI_ONE:
                this.showHolds();
                this.setBet(data.bet, data.shemen);
                this.updateResult(this.holdsData, data.holdsValue);
                break;
            default:
                break
        }
     ...
     * @param {*} guodi 
     */
    setScore (score, ani, guodi) {
        if (typeof score == 'number') {
            this._score = score;
        }
        
        if (typeof guodi == 'number') {
            this._guodi = guodi;
        }

        this.scoreNode.getComponent('scoreAni').showNum(this._score-this._guodi, ani);
    },
    ...
    setBet: function (score, shemen) {
        ...
        this.betNode.getComponent('bet').show(score, null, shemen);
    },
     ...
     * @param score 移动多少分
     */
    showFinalScore (guodi) {
        if (!this.node.active) {
            return;
        }

        if (this.finalScoreData !== undefined) {
            this.setScore(this.finalScoreData, true, guodi);
        }
    },
    ...
    updateResult (holds, holdsValue) {
        ...
        let nx = this._resorix;
        let margin = 0;
        if (b.length > 0) {
            margin = this.localSeat == 0 ? 20 : 16;
            nx -= margin/2;
        }
