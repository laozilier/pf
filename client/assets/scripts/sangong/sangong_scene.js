...
cc.Class({
    ...
    onRoomInfo(data) {
        this._super(data);
        if (this.playLab) {
            this.playLab.string = this._model._game_rule.playMode ? '玩法:三公比金花' : '玩法:经典三公';
        }
    ...
    gameResult (data) {
        ...
        let self = this;
        this._myReady = false;
        this.unschedule(this.showButton);
        this.unschedule(this.showLiangPaiButton);
        //游戏结束将托管按钮隐藏设置为换桌按钮
        let c = data[2];
        if (c == undefined) {
            ...
            if (this.isPlaying() && !this.isTourist()) {
                if (this._model._game_rule.playMode > 0) {
                    if (c > 0) {
                        this.startEndnode();
                    }
                } else {
        ...
        if (this._model._game_rule.playMode > 0) {
            this._model._uids.forEach(uid => {
                ...
                if (!!p) {
                    ...
                    ps.updateResult(ps.holdsData, ps.typeData, c);
                }
            });
        }
        ...
        let tipsStr = (c == 1 ? '金花比牌中: ' : '三公比牌中: ');
        this.setTips(tipsStr, 3);
    ...
    onCuoPai: function (event) {
        ...
        if(!!p){
            ...
            //顶部四张牌
            if (selfScript) {
                let holds = selfScript.holdsData.slice(0, 2);
